...
<ul class="pagination">
	<li data-page=1><c:if test="${pageObject.page > 1 }">
			<a
				href="${listURI }?page=1&perPageNum=${pageObject.perPageNum}${query}"
	...
	<li data-page=${pageObject.startPage -1 }><c:if
			...
			<a
				href="${listURI }?page=${pageObject.startPage - 1 }&perPageNum=${pageObject.perPageNum}${query}"
	...
	<c:forEach begin="${pageObject.startPage }"
		...
		<li ${(pageObject.page == cnt)?"class=\"active\"":"" }
			...
			<!-- 페이지와 cnt가 같으면 링크가 없음 --> <c:if test="${pageObject.page == cnt }">
				...
				<a
					href="${listURI }?page=${cnt }&perPageNum=${pageObject.perPageNum}${query}"
	...
	<c:if test="${pageObject.endPage < pageObject.totalPage }">
		<li data-page=${pageObject.endPage + 1 }><a
			href="${listURI }?page=${pageObject.endPage + 1 }&perPageNum=${pageObject.perPageNum}${query}"
	...
	<c:if test="${pageObject.page < pageObject.totalPage }">
		<li data-page=${pageObject.totalPage }><a
			href="${listURI }?page=${pageObject.totalPage }&perPageNum=${pageObject.perPageNum}${query}"
